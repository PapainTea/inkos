    ...
    try {
      ...
      if (!isSubplotsSentinel(reviseOutput.updatedSubplots)) {
        ...
        const mergedSubplots = currentSubplots
          ? mergeTableMarkdownByKey(currentSubplots, reviseOutput.updatedSubplots, [0])
      ...
      if (!isEmotionalArcsSentinel(reviseOutput.updatedEmotionalArcs)) {
        ...
        const mergedEmoArcs = currentEmoArcs
          ? mergeTableMarkdownByKey(currentEmoArcs, reviseOutput.updatedEmotionalArcs, [0, 1])
      ...
      if (!isCharacterMatrixSentinel(reviseOutput.updatedCharacterMatrix)) {
        ...
        const mergedMatrix = currentMatrix
          ? mergeCharacterMatrixMarkdown(currentMatrix, reviseOutput.updatedCharacterMatrix)
      ...
      if (!isChapterSummariesSentinel(reviseOutput.updatedChapterSummaries)) {
        ...
        const mergedSummaries = mergeChapterSummariesMarkdown(
          currentSummaries,
          reviseOutput.updatedChapterSummaries,
        );
        await writeFile(join(storyDir, "chapter_summaries.md"), mergedSummaries, "utf-8");
      }
...
${matrix}`,
  ...
  async rebuildHooksFromChapters(
    ...
    const readSafe = async (p: string): Promise<string> => readFile(p, "utf-8").catch(() => "");
    const storyDir = join(bookDir, "story");
    const [currentState, particleLedger, chapterSummaries, subplotBoard, emotionalArcs, characterMatrix, volumeOutline] =
      await Promise.all([
        readSafe(join(storyDir, "current_state.md")),
        readSafe(join(storyDir, "particle_ledger.md")),
    ...
    for (const [index, chapter] of chapters.entries()) {
      ...
      const response = await chatCompletion(this.config.client, this.config.model, [
        {
          role: "system",
          content: buildSettlerSystemPrompt(book, gp, bookRules, language),
        },
        {
          role: "user",
          content: buildSettlerUserPrompt({
            chapterNumber: chapter.chapterNumber,
            title: chapter.title,
  ...
  async rebuildLedgerFromChapters(
    ...
    const readSafe = async (p: string): Promise<string> => readFile(p, "utf-8").catch(() => "");
    const storyDir = join(bookDir, "story");
    const [currentState, pendingHooks, chapterSummaries, subplotBoard, emotionalArcs, characterMatrix, volumeOutline] =
      await Promise.all([
        readSafe(join(storyDir, "current_state.md")),
        readSafe(join(storyDir, "pending_hooks.md")),
        readSafe(join(storyDir, "chapter_summaries.md")),
